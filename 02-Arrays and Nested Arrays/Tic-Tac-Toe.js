function TicTacToe(input) {
    ...
    for (let el of input) {
        ...
        } else if (dashboard[0].includes('false') == true || dashboard[1].includes('false') == true || dashboard[2].includes('false') == true) {
            console.log('This place is already taken. Please choose another!');
            continue;
        }
        if (dashboard[0].includes('false') == false && dashboard[1].includes('false') == false && dashboard[2].includes('false') == false) {
        ...
        for (let i = 0; i < dashboard.length - 1; i++) {
            ...
            if (dashboard[i][dashboard.length - i - 1] == 'false') {
                checker = false;
                break;
            } else if (dashboard[i][dashboard.length - i - 1] !== dashboard[i + 1][dashboard.length - i - 2]) {
                checker = false;
                break;
            }
        }
